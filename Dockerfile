...
RUN apt-get update && apt-get install --no-install-recommends -y \
    ca-certificates iproute2 postgresql-$PG_MAJOR_VERSION-repmgr curl bash dnsutils vim socat procps ssh gnupg rsync barman-cli barman barman-cli-cloud cron postgresql-$PG_MAJOR_VERSION-cron \
    && apt autoremove -y
